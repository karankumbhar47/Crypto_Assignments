...
\textbf{8-bit Attack}
    \begin{enumerate}
        ...
        the Sypher004, up to round 4 output.
            ...
            \end{center}
        total cost of this path is $2 \times (\frac{6}{14})^4$ (probability that our key
        guess is correct)

        \textbf{Note}: We are analyzing all difference path to get most optimum path 
        for our attack. For analyzing we are using following function form \texttt{utils.py}
            \begin{itemize}
                \item \texttt{generateDiffPath}
                \item \texttt{getAllPath}
                \item \texttt{get\_all\_candidates}
            \end{itemize}
        \item As we know the initial message pair difference and difference at the end
        of four round which is $(0,2,2,0)$. 
        ...
        \item All This code is automated in \texttt{attack.py} file (it will take more than 15 min to get correct key)
        \item Faster version of automated code is in file \texttt{multi\_thread.py} using multithreading. (around $10-12$ min)
        \end{enumerate}
        ...
        \textbf{Output Image }
        \begin{center}
            \includegraphics[width=5.2cm]{./q1.png}
        \end{center}
