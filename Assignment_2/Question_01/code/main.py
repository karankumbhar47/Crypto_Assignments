...
def main():
    key = [hex(ord(c))[2:].zfill(2) for c in "STARTCTF2XPANDME"]
    w = keyExpansion(key)
    ...
    for mat in matrix:
        str = ""
        for key in mat:
            binary_num = bin(int(key, 16))[2:]
            binary_num = binary_num.zfill(8)
            str += binary_num
        keys.append(str)

    # keys get from Sypher00B
    guessed_keys = Sypher00B.main()
    key_str = ""
    ...
    for key in keys:
        ...
        last = key[-17:]+k0k1k2
        curr = [first, last]
        # concatinate 15 bits to 17 bits from key expansion
        seventeen.append(curr)

    print("\n Final 32 bit Key Guess\n")
    for i, seven in enumerate(seventeen):
        print(f"key {i}")
        print(seven[0])
        print(seven[1])
