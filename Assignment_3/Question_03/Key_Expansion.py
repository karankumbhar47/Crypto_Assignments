from Utils import *

# AES S-Box
Sbox = [
    
]

# Round constants
RoundConst = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]

# Initial key
initial_key = [
    [0xB3, 0x2F, 0x2F, 0x63],
    [0xB7, 0x3B, 0x63, 0x83],
    [0x29, 0x63, 0x00, 0xED],
    [0x63, 0x83, 0xFC, 0xB7]
]

round5_6_steps = []


def sub_col(col):
    return [Sbox[item] for item in col]


def rotate_col(col):
    return col[1:]+[col[0]]


def xor_cols(word1, word2):
    return [b1 ^ b2 for b1, b2 in zip(word1, word2)]


def key_expansion(key):
    all_keys = [key]

    for i in range(10):
        previous_key = transpose_key(all_keys[i])
        prev_last_col = previous_key[-1]
        prev_last_col_rot = rotate_col(prev_last_col)
        prev_last_col_sub = sub_col(prev_last_col_rot)
        round_const_arr = [RoundConst[i], 0, 0, 0]
        prev_last_col_xor = xor_cols(prev_last_col_sub, round_const_arr)

        if (i == 5):
            round5_6_steps.append(prev_last_col)
            round5_6_steps.append(prev_last_col_rot)
            round5_6_steps.append(prev_last_col_sub)
            round5_6_steps.append(round_const_arr)
            round5_6_steps.append(prev_last_col_xor)

        new_key = []
        for j in range(4):
            new_key.append(xor_cols(prev_last_col_xor if j ==
                           0 else new_key[j-1], previous_key[j]))
            if i == 5:
                round5_6_steps.append(
                    prev_last_col_xor if j == 0 else new_key[j-1])
                round5_6_steps.append(previous_key[j])
                round5_6_steps.append(new_key[-1])

        all_keys.append(transpose_key(new_key))
    return all_keys


def main():
    write_initialKey(initial_key)

    print("\nOutput Will be updated in PDF (Better Representation in PDF)\n")

    round_keys = key_expansion(initial_key)
    for i, key in enumerate(round_keys):
        print_key(key, i)

    print("\nOutput Will be updated in PDF (Better Representation in PDF)\n")

    write_roundKeys(round_keys[1:])
    write_intermediate_steps(round_keys, round5_6_steps)


if __name__ == "__main__":
    main()
