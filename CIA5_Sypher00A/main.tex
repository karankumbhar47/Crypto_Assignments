...
\newpage
\begin{Problem}
    \includegraphics[width=15.2cm]{./q.png}
...
\begin{Solution}
    ...
    \begin{enumerate}
        \item First we will find message and cipher text pair using \texttt{messageCipher.py} file
        \item Then we will select 8 pairs of $\alpha$ and $\beta$ masks form the \texttt{Linear Approximation Table}
        ...
        \item After putting all this variables and bitwise operation as per equation(1), we will get  Linear equations.
        \item After Solving this linear equation We will get value for each variable from which we can find key value.
    \end{enumerate}
All this process is automated by the python file \texttt{automate.py}. 
